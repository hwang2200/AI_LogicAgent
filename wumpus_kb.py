...
def axiom_generator_percept_sentence(t, tvec):
    ...
    for i in range(len(tvec)):
        if tvec[i] == False:
            ...
            prefix = ''
        result.append(prefix + percepts[i] + str(t))
...
def axiom_generator_initial_location_assertions(x, y):
    ...
    """
    axiom_str = ''
    result = ['~' + wumpus_str(x, y), '~' + pit_str(x,y)]
    axiom_str = ' & '.join(result)
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    #All pit locations if not boundary
    if x - 1 >= xmin:
        rhs.append(pit_str(x - 1, y))
    if x + 1 <= xmax:
        rhs.append(pit_str(x + 1, y))
    if y - 1 >= ymin:
        rhs.append(pit_str(x, y - 1))
    if y + 1 <= ymax:
        rhs.append(pit_str(x, y + 1))

    #"(or the same location!)"
    rhs.append(pit_str(x, y))

    axiom_str = lhs + '(' + ' | '.join(rhs) + ')'
    return axiom_str
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    #All possible Wumpus locations if not at wall
    if x - 1 >= xmin:
        rhs.append(wumpus_str(x - 1, y))
    if x + 1 <= xmax:
        rhs.append(wumpus_str(x + 1, y))
    if y - 1 >= ymin:
        rhs.append(wumpus_str(x, y - 1))
    if y + 1 <= ymax:
        rhs.append(wumpus_str(x, y + 1))

    #"(or the same location!)"
    rhs.append(wumpus_str(x, y))
...
def axiom_generator_at_least_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    #For every location, Wumpus can only be in one
    for i in range(xmin, xmax + 1):
        for j in range(ymin, ymax + 1):
            result.append(wumpus_str(i, j))
    axiom_str = '(' + ' | '.join(result) + ')'
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_at_most_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    axiom_str = ''
    firstPairs = []
    #For every pair of locations, Wumpus is not at either
    for i in range(xmin, xmax + 1):
        for j in range(ymin, ymax + 1):
            firstPairs.append('~' + wumpus_str(i, j))
    
    result = []
    for i in range(len(firstPairs)):
        for j in range(i + 1, len(firstPairs)):
            result.append('(' + firstPairs[i] + ' | ' + firstPairs[j] + ')')
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t = 0):
    ...
    #If agent is at (xi, yi), it cannot be at any other location
    for i in range(xmin, xmax + 1):
        for j in range(ymin, ymax + 1):
            #print(i, j)
            if i != xi or j != yi:
                result.append('~' + state_loc_str(i, j, t))
    
    result.append(state_loc_str(xi, yi, t))
...
def axiom_generator_only_one_heading(heading = 'north', t = 0):
    ...
    if heading == 'north':
        conditions = [state_heading_north_str(t), '~' + state_heading_east_str(t), '~' + state_heading_south_str(t), '~' + state_heading_west_str(t)]
    if heading == 'east':
        conditions = [state_heading_east_str(t), '~' + state_heading_north_str(t), '~' + state_heading_south_str(t), '~' + state_heading_west_str(t)]
    if heading == 'south':
        conditions = [state_heading_south_str(t), '~' + state_heading_north_str(t), '~' + state_heading_east_str(t), '~' + state_heading_west_str(t)]
    if heading == 'west':
        conditions = [state_heading_west_str(t), '~' + state_heading_north_str(t), '~' + state_heading_south_str(t), '~' + state_heading_east_str(t)]

    axiom_str = ' & '.join(conditions)
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_have_arrow_and_wumpus_alive(t = 0):
    ...
    """
    axiom_str = ''
    result = [state_have_arrow_str(t), state_wumpus_alive_str(t)]
    axiom_str = ' & '.join(result)
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_location_OK(x, y, t):
    ...
    lhs = state_OK_str(x, y, t) + ' <=> '

    condPit = '~' + pit_str(x, y) 
    condWumpus = ['~' + wumpus_str(x, y), '~' + state_wumpus_alive_str(t)]
    rhs = '(' + ' | '.join(condWumpus) + ')'
    
    axiom_str = lhs + '(' + condPit + ' & ' + rhs + ')'
    return axiom_str
...
def axiom_generator_breeze_percept_and_location_property(x, y, t):
    ...
    """
    axiom_str = ''
    startLoc = state_loc_str(x, y, t) + ' ==> '
    axiom_str = startLoc + '(' + percept_breeze_str(t) + ' <=> ' + breeze_str(x, y) + ')'
    return axiom_str
...
def axiom_generator_stench_percept_and_location_property(x, y, t):
    ...
    axiom_str = ''
    startLoc = state_loc_str(x, y, t) + ' ==> '
    axiom_str = startLoc + '(' + percept_stench_str(t) + ' <=> ' + stench_str(x, y) + ')'
    return axiom_str
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    """
    axiom_str = ''
    initialTime = state_loc_str(x, y, t)
    nextTime = state_loc_str(x, y, t + 1) + ' <=> '
    actions = []

    #Which action caused it
    if x - 1 >= xmin:
        conditions = [state_loc_str(x - 1, y, t), state_heading_east_str(t), action_forward_str(t)]
        actions.append('(' + ' & '.join(conditions) + ')')
    if x + 1 <= xmax:
        conditions = [state_loc_str(x + 1, y, t), state_heading_west_str(t), action_forward_str(t)]
        actions.append('(' + ' & '.join(conditions) + ')')
    if y - 1 >= ymin:
        conditions = [state_loc_str(x, y - 1, t), state_heading_north_str(t), action_forward_str(t)]
        actions.append('(' + ' & '.join(conditions) + ')')
    if y + 1 <= ymax:
        conditions = [state_loc_str(x, y + 1, t), state_heading_south_str(t), action_forward_str(t)]
        actions.append('(' + ' & '.join(conditions) + ')')

    #F^{t+1} <=> (Action(s)ThatCause_F^t)
    first = nextTime + '(' + ' | '.join(actions)

    #What didn't cause it (F^t & ~Action(s)ThatCauseNot_F^t) -> must be at initial location and not move forward for each action
    inactions = [action_grab_str(t), action_shoot_str(t), action_climb_str(t), action_turn_left_str(t), action_turn_right_str(t), action_wait_str(t), percept_bump_str(t + 1)]
    second = '(' + initialTime + ' & ' + '~' + action_forward_str(t) + ' & ' '(' + ' | '.join(inactions) + ')' + ')'
    axiom_str = first + ' | ' + second + ')'
...
def axiom_generator_have_arrow_ssa(t):
    ...
    """
    axiom_str = ''
    lhs = state_have_arrow_str(t + 1) + ' <=> ' 

    #If the agent still has the arrow at t + 1, they had the arrow at t and didn't shoot
    conditions = [state_have_arrow_str(t), '~' + action_shoot_str(t)]
    axiom_str = lhs + '(' + ' & '.join(conditions) + ')'
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_wumpus_alive_ssa(t):
    ...
    """
    axiom_str = ''
    lhs = state_wumpus_alive_str(t + 1) + ' <=> '

    #Wumpus is alove and no scream is detected at next time step (due to note above)
    conditions = [state_wumpus_alive_str(t), '~' + percept_scream_str(t + 1)]
    axiom_str = lhs + '(' + ' & '.join(conditions) + ')'
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_heading_north_ssa(t):
    ...
    """
    axiom_str = ''
    lhs = state_heading_north_str(t + 1) + ' <=> '

    #Heading north already, heading east and turning left, heading west and turning right
    conditions = []
    #For readability
    conditions.append(state_heading_north_str(t) + ' & ' + '~' + action_turn_right_str(t) + ' & ' + '~' + action_turn_left_str(t))
    conditions.append(state_heading_west_str(t) + ' & ' + action_turn_right_str(t))
    conditions.append(state_heading_east_str(t) + ' & ' + action_turn_left_str(t))
...
def axiom_generator_heading_east_ssa(t):
    ...
    """
    axiom_str = ''
    lhs = state_heading_east_str(t + 1) + ' <=> '

    #Heading east already, heading south and turning left, heading north and turning right
    conditions = []
    #For readability
    conditions.append(state_heading_east_str(t) + ' & ' + '~' + action_turn_right_str(t) + ' & ' + '~' + action_turn_left_str(t))
    conditions.append(state_heading_north_str(t) + ' & ' + action_turn_right_str(t))
    conditions.append(state_heading_south_str(t) + ' & ' + action_turn_left_str(t))
...
def axiom_generator_heading_south_ssa(t):
    ...
    """
    axiom_str = ''
    lhs = state_heading_south_str(t + 1) + ' <=> '

    #Heading south already, heading east and turning right, heading west and turning left
    conditions = []
    #For readability
    conditions.append(state_heading_south_str(t) + ' & ' + '~' + action_turn_right_str(t) + ' & ' + '~' + action_turn_left_str(t))
    conditions.append(state_heading_west_str(t) + ' & ' + action_turn_left_str(t))
    conditions.append(state_heading_east_str(t) + ' & ' + action_turn_right_str(t))
...
def axiom_generator_heading_west_ssa(t):
    ...
    """
    axiom_str = ''
    lhs = state_heading_west_str(t + 1) + ' <=> '

    #Heading west already, heading south and turning right, heading north and turning left
    conditions = []
    #For readability
    conditions.append(state_heading_west_str(t) + ' & ' + '~' + action_turn_right_str(t) + ' & ' + '~' + action_turn_left_str(t))
    conditions.append(state_heading_north_str(t) + ' & ' + action_turn_left_str(t))
    conditions.append(state_heading_south_str(t) + ' & ' + action_turn_right_str(t))
...
def axiom_generator_heading_only_north(t):
    ...
    """
    axiom_str = ''
    lhs = state_heading_north_str(t) + ' <=> '
    conditions = []
    conditions.append('~' + state_heading_east_str(t))
    conditions.append('~' + state_heading_south_str(t))
    conditions.append('~' + state_heading_west_str(t))
...
def axiom_generator_heading_only_east(t):
    ...
    """
    axiom_str = ''
    lhs = state_heading_east_str(t) + ' <=> '
    conditions = []
    conditions.append('~' + state_heading_south_str(t))
    conditions.append('~' + state_heading_west_str(t))
    conditions.append('~' + state_heading_north_str(t))
...
def axiom_generator_only_one_action_axioms(t):
    ...
    allActions = '(' + ' | '.join(actions) + ')'

    actionPairs = []
    #For each pair of actions, both cannot happen at the same time
    for i in range(len(actions)):
        for j in range(i + 1, len(actions)):
            actionPairs.append('(' + '~' + actions[i] + ' | ' + '~' + actions[j] + ')')

    axiom_str = allActions + ' & ' + ' & '.join(actionPairs)
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
